...
class SmsParser {
  ...
  static Record? parsePostalSavings(String sms) {
    if (!sms.contains('邮储银行')) return null;

    final isExpense = sms.contains('支出金额');
    final isIncome = sms.contains('收入金额') && sms.contains('汇入');

    if (!isExpense && !isIncome) return null;

    // 金额
    final amountMatch = RegExp(r'[支收]入金额(\d+\.?\d*)元').firstMatch(sms);
    ...
    // 日期: 26年05月29日11:21
    DateTime? date;
    final dateMatch = RegExp(r'(\d+)年(\d+)月(\d+)日(\d+):(\d+)').firstMatch(sms);
    if (dateMatch != null) {
    ...
    // 分类
    String category = '其他';
    if (sms.contains('快捷支付')) category = '购物';
    if (sms.contains('支付宝')) category = '购物';
    if (sms.contains('汇入')) category = '工资';
  ...
  static Record? parse(String sms) {
    if (sms.contains('邮储银行')) {
      return parsePostalSavings(sms);
    }
    return null;
  }
}